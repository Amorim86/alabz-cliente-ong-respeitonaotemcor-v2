---
name: alabz-lapidacao-pre-deploy
description: Comando de pré-deploy para blindagem e execução das 5 skills estratégicas em cascata antes da publicação.
---

# Comando de Pré-Deploy (/alabz-lapidacao-pre-deploy)

Se o usuário digitar `/alabz-lapidacao-pre-deploy` em qualquer momento ou acionar esta skill, você deve imediatamente:
...
1. **Alinhamento de Funcionalidades (CTA & Cookies/LGPD)**: Antes de iniciar a blindagem em cascata, confirme com o usuário ou valide na base de código se o projeto necessita de formulários de captação (CTA) e/ou banner de consentimento de cookies (LGPD):
   - **Formulários de CTA**: Valide se a skill `/alabz-componentes-formulario-cta` foi executada, se os formulários específicos foram gerados com o `clienteId` correto do projeto e se as variáveis de ambiente necessárias estão devidamente listadas/mapeadas.
